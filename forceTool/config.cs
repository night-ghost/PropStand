...
namespace OSD {
    ...
    internal unsafe struct EEPROM {
        ...
        // битовые флаги
        [FieldOffset(Stand.Settings_offset)] // 512 - 4 экрана по 128, остальное для настроек
        internal Flags flags;
        // прочие настройки
        [FieldOffset(Stand.Settings_offset + 4)]
        internal Settings sets;

        [FieldOffset(Stand.Settings_offset + 128)]        
        internal Sensors sensors;


        [FieldOffset(Stand.Settings_offset + Config.Settings_size )]
    ...
    internal class Config {
        ...
        const int Settings_offset = Stand.Settings_offset; // 512 - половина
        const int OffsetBITpanel = Stand.OffsetBITpanel;
        ...
        private Stand osd;// ссылка на родителя для удобия доступа

        // методы
        public Config(Stand aosd) {
            osd = aosd;
            eeprom = new EEPROM();
            eeprom.clear();
        }
        ...
        public void setEepromScrFlags(uint16_t val) {
            ...
            eeprom[osd.screen_number * Stand.OffsetBITpanel + pan_pos] = (byte)(val & 0xFF); // x
            eeprom[osd.screen_number * Stand.OffsetBITpanel + pan_pos + 1] = (byte)(val>>8); // y
        }

        public uint16_t getEepromScrFlags() {
            uint16_t val;
            const int pan_pos = 0; // position of flags
            val  = (uint16_t)(eeprom[osd.screen_number * Stand.OffsetBITpanel + pan_pos]); // x
            val |= (uint16_t)(eeprom[osd.screen_number * Stand.OffsetBITpanel + pan_pos + 1] <<8); // y
            return val;
        }
        ...
        public void setEepromXY(Panel pan, bool enabled) {
            ...
            eeprom[osd.screen_number * Stand.OffsetBITpanel + pan.pos] = (byte)(x | fSign | flag4); // x
            eeprom[osd.screen_number * Stand.OffsetBITpanel + pan.pos + 1] = (byte)((enabled ? y : y | 0x80) | flag | flag2 | flag3); // y
        }

        public Pos getEepromXY(Panel pan) {
            return new Pos(eeprom[osd.screen_number * Stand.OffsetBITpanel + pan.pos],
                            eeprom[osd.screen_number * Stand.OffsetBITpanel + pan.pos + 1]);
        }
